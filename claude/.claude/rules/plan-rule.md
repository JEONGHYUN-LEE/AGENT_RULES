...
## Process
1. **Goal analysis** (from knowledge): interpret goal, identify relevant modules/files, current behavior, execution flow, constraints, invariants, dependencies, edge cases, risks.
2. **Plan design**: decide approach, modification boundary, steps, validation, mitigations.

## `docs/plan.md` must include

1. Goal (and non-goals)
2. Relevant context (reference all .md files under `docs/knowledge/`, avoid re-explaining codebase)
3. Design options considered → chosen approach
4. **Modification boundary**: files allowed to change / small edits only / tests
